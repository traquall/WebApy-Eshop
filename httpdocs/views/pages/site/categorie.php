...
<div class="container">
    <div class="row">
      <div class="grid1column">
        <h3><?php   \Noneslad\Controllers\categorieController::findAll();
        echo $categorie['Nom']; ?></h3>
      </div>
      <?php $content = $categorie->getContenu();
      $col = 4;
          ...
          <div class="grid4column">
            ...
            foreach (\Noneslad\Controllers\categorieController::sousmenu($id_nav) as $categorie4) { 
              if($j==0){ ?>
                ...
                <?php } ?>
                  <li class="cat">
                    <a href="index.php?page=admin&crud=categorie&action=read&id=<?php echo $categorie4['id']; ?>" <?php if($categorie4['id']==$_GET["id"]){ echo 'class="active"'; } ?>><?php echo $categorie4['nom']; ?></a>
                  </li>
              <?php $j++; } 
              if($j>0){ ?>
                </ul>
    ...
    foreach (\Noneslad\Controllers\categorieController::doc_c() as $document) { 
      if($i==0){ ?>
      <h4><?php if($_SESSION["langue"]==1){ ?>T&eacute;l&eacute;chargements<?php } else {?>Downloads<?php } ?></h4>
      <?php } 
